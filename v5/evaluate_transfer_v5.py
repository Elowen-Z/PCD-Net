# -*- coding: utf-8 -*-
"""
v5 迁移学习评估脚本 evaluate_transfer_v5.py
基于 v3/evaluate_transfer.py 适配 NoiseAwareDenoiserV5。

支持 --ckpt / --output_dir 覆盖，方便同时跑 transfer / baseline 两次。
"""
...
# ============================================================
CONFIG = {
    "ckpt_path":    r"D:/X/denoise/part1/v5/checkpoints_transfer_v5/best_transfer_v5.pth",
    ...
    "output_dir":   r"D:/X/denoise/part1/v5/eval_transfer_v5",
    "max_samples":  15000,
    "val_ratio":    0.1,
    "signal_len":   6000,
    "cond_len":     400,
...
# ============================================================
# Dataset (与 v3/evaluate_transfer.py 一致)
# ============================================================
class EvalDataset(Dataset):
    ...
    def _get_p_sample(self, row):
        for col in ["trace_P_arrival_sample", "p_arrival_sample",
                    "p_onset", "itp", "Pg"]:
...
def cc_fn(clean, test):
    c = clean - clean.mean(); t = test - test.mean()
...
def evaluate(model, loader, device):
    ...
    records, quality_list = [], []
    pick_err_noisy, pick_err_denoise = [], []
    pick_suc_noisy, pick_suc_denoise = [], []
    skip = 0
    ...
    for batch in loader:
        ...
        try:
            # v5 forward 返回 4-tuple (clean, quality, z_noise, aux)
            pred, quality, _, _ = model(noisy, z_cond)
        ...
        for i in range(noisy.shape[0]):
            ...
            records.append({
                ...
                "quality":         qual,
                "snr_group":       assign_snr_group(snr_i),
            })
            ...
            p_norm = p_np.copy()
            peak = np.abs(p_norm).max()
            if peak > 1e-10:
                p_norm = p_norm / peak
            pick_d = stalta_pick(p_norm, **kw)
...
# 汇总
# ============================================================
def summarize(records, tag="transfer"):
...
# 图
# ============================================================
def plot_snr_summary(df, output_dir, tag):
    color_map = {
        "<-5": "#d62728", "-5~0": "#ff7f0e", "0~5": "#2ca02c",
        "5~10": "#1f77b4", "10~15": "#9467bd", ">15": "#8c564b",
    }
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle(f"V5 {tag} — SNR Analysis", fontsize=12, fontweight="bold")
    ...
    ax.set_xlabel("Input SNR (dB)"); ax.set_ylabel("Output SNR (dB)")
    ax.set_title("Input vs Output SNR"); ax.legend(title="Group", fontsize=8)
    ax.grid(alpha=0.3)
    ...
    ax.set_xlabel("SNR Group (Input)"); ax.set_ylabel("Mean SNR Gain (dB)")
    ax.set_title("Per-Group SNR Gain"); ax.grid(axis="y", alpha=0.3)
    for bar, m in zip(bars, means):
        ax.text(bar.get_x() + bar.get_width()/2,
                m + (0.1 if m >= 0 else -0.4),
                f"{m:+.2f}", ha="center", fontsize=8)
    plt.tight_layout()
    out = os.path.join(output_dir, f"v5_{tag}_snr_summary.png")
    plt.savefig(out, dpi=150, bbox_inches="tight"); plt.close(fig)
    print(f"  [图] {out}")


def plot_waveform_samples(model, loader, device, output_dir, tag, n=12):
    model.eval()
    saved = 0
    fig, axes = plt.subplots(n, 3, figsize=(18, n * 2))
    fig.suptitle(f"V5 {tag} — Waveform Samples (Z channel)", fontsize=11, fontweight="bold")
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for i in range(noisy.shape[0]):
                ...
                for a in ax:
                    a.set_xlabel("Time (s)", fontsize=7); a.tick_params(labelsize=7)
                    a.grid(alpha=0.3)
                saved += 1

    plt.tight_layout()
    out = os.path.join(output_dir, f"v5_{tag}_waveform_samples.png")
...
def parse_args():
    ...
    ap.add_argument("--ckpt",       default=CONFIG["ckpt_path"])
...
def main():
    ...
    for k in ["ckpt_path", "finetune_h5", "finetune_csv", "noise_h5", "noise_csv"]:
        p = CONFIG[k]
        print(f"  [{'OK' if os.path.exists(p) else 'XX'}] {k}: {p}")
    ...
    print(f"  参数总数  : {n_params/1e6:.2f} M")

    print(f"\n[评估中... tag={CONFIG['tag']}]")
    records, pick_results = evaluate(model, loader, device)

    df, summary = summarize(records, tag=CONFIG["tag"])
    pick_stats  = print_pick_stats(pick_results)

    tag = CONFIG["tag"]
    df.to_csv(os.path.join(CONFIG["output_dir"], f"v5_{tag}_results.csv"),
              index=False, float_format="%.4f")
    pd.DataFrame([{**summary, **pick_stats, "tag": tag,
                   "ckpt": CONFIG["ckpt_path"]}]).to_csv(
        os.path.join(CONFIG["output_dir"], f"v5_{tag}_summary.csv"),
        index=False, float_format="%.4f")
    pd.DataFrame({
    ...
    }).to_csv(os.path.join(CONFIG["output_dir"], f"v5_{tag}_pick_detail.csv"),
              index=False, float_format="%.4f")

    plot_snr_summary(df, CONFIG["output_dir"], tag)
    plot_waveform_samples(model, loader, device,
                          CONFIG["output_dir"], tag, n=CONFIG["save_fig_num"])

    print(f"\n[OK] 结果目录: {CONFIG['output_dir']}")
